package com.company.thesissandbox.core.app;
...
@ManagedBean("thesis-sandbox_ExtDeployer")
public class ExtDeployer extends AbstractDeployer implements ExtDeployerMBean {
    ...
    protected void checkForFirstInit() {
        ...
        try {
            ...
            List<Config> configs = em.createQuery("select c from sys$Config c where c.name='thesis-sandbox.initDefaultInFirstStart'",
                ...
                if (StringUtils.isNotBlank(edition) && Integer.parseInt(edition) > 1) {
                    ...
                    if (initDefaultInFirstStart == null) {
                        try {
                            ...
                            config.setName("thesis-sandbox.initDefaultInFirstStart");
                        ...
                        try {
                            login();
                            em.createQuery("update sys$Config c set c.value='false' where c.name='thesis-sandbox.initDefaultInFirstStart'",
    ...
    protected void executeInitScripts() {
        ...
        File folderDB = new File(dbDirPath + "/50-thesis-sandbox/init/" + dbmsType);
